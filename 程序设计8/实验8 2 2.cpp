...
int main()
{
	...
	int savebest();
	int save_1();
	printf("将从input.txt文件中输入%d位同学的信息，包括：学号（4位）、姓名、课程1—3成绩\n",n);
...
int save()
{
	FILE *fp;
	int i;
	if((fp = fopen("文件输入grade.txt","w+"))==NULL)
	{
		printf("cannot open grade.txt\n");
		exit(0);
	}
	fprintf(fp,"%s\t\n","以下是未插入信息后按平均分由高到低排序（文件输入）");
...
{
	FILE *fp;
	if((fp = fopen("文件输入gradebest.txt","w+"))==NULL)
	{
		printf("cannot open grade.txt\n");
		exit(0);
	}
	fprintf(fp,"%s\t\n","以下是未重新排序前成绩最优秀的学生（文件输入）");
	fprintf(fp,"%s\t%s\t%s\t%s\t%s\t%s\n","姓名","学号","课程1","课程2","课程3","平均分");
...
int save_1()
{
	FILE *fp;
	int i;
	if((fp = fopen("文件输入grad_1.txt","w+"))==NULL)
	{
		printf("cannot open grade.txt\n");
		exit(0);
	}
	fprintf(fp,"%s\t\n","以下是重新排序后按平均分由高到低排序（文件输入）");
	fprintf(fp,"%s\t%s\t%s\t%s\t%s\t%s\n","姓名","学号","课程1","课程2","课程3","平均分");
	for(i=0;i<n+1;i++)
	{
	fprintf(fp,"%s\t%s\t%.1f\t%.1f\t%.1f\t%.1f\t\n",stu[i].name,stu[i].num,stu[i].c_1,stu[i].c_2,stu[i].c_3,stu[i].average);
